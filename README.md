...
<h1>Последовательность действий для установки на новый сервер</h1>
   Я максимально постарался автоматизировать процесс установки и настройки
   
    _Скрипты позволяют устанавливать все пакеты из локальной папки. Для этого необходимо поместить в папку /home/administrator/distr/ _
...
6. Установка zabbix-agent **install_zabbix_agent.sh**
   Для подключения мониторинга Zabbix зайти на http://zbxsrv/zabbix и добавить станцию
7. Установка мониторинга ELK запустить **install_filebeat.sh**
      _предварительно необходимо загрузить из distrib filebeat-9.4.4-amd64.deb_
